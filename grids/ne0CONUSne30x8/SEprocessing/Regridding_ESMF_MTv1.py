...
MODIFICATION HISTORY:
    ...
    Duseong Jo, 03, MAR, 2022: VERSION 7.01
    - Include more types for molecular weight check
    Duseong Jo, 27, SEP, 2022: VERSION 7.10
    - Add check for lat bounds
    Duseong Jo, 23, FEB, 2023: VERSION 7.20
    - Include more time types for checking
    Duseong Jo, 28, FEB, 2023: VERSION 7.30
    - To deal with some cases with "bounds" in some emission files
'''
...
class Add_bounds(object):
    ...
    def __init__(self, filename, newfilename=None, creation_date=True,
        ...
        latvar.setncattr( 'comment', 'center_of_cell' )
        ...
        lonvar.setncattr( 'comment', 'center_of_cell' )
        ...
        # === make sure lat values are within -90 ~ 90
            ...
            if mw == None:
                ...
                for fld in self.fields_file:
                    if 'molecular_weight' in self.var_array[fld].attrs.keys():
                        ...
                        elif type( self.var_array[fld].molecular_weight ) in [int, float, np.float32, np.float64, np.int32, np.int64]:
                            self.mw = np.copy( self.var_array[fld].molecular_weight ).astype('f')
                        else:
                            raise ValueError( 'Check the type of molecular weight value!\n' + \
                                              'Currently those are supported: str, int, float, np.float32, np.float64, np.int32, np.int64' )
        ...
        if not self.speed_up:   
            ...
            if self.save_results:
                ...
                for afld in self.add_fields:
                    for adim in self.var_array[afld].dims:
                        if adim in self.dst_dim:
                            continue
                        else:
                            fid.createDimension( adim, len(self.var_array[afld][adim]) )                            
            ...
            if self.add_fields != []:
                for afld in self.add_fields:
                    var_tmp = fid.createVariable( self.var_array[afld].name, np.dtype( self.var_array[afld].values.flat[0] ).name,
                                                  self.var_array[afld].dims )
                    var_tmp[:] = self.var_array[afld].values[:]
                    for key in list( self.var_array[afld].attrs.keys() ):
                        var_tmp.setncattr( key, self.var_array[afld].attrs[key] )
