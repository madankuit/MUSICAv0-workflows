...
MODIFICATION HISTORY:
    ...
    Madankui Tao, 6, March, 2023
    - modified map settings for individual project
    Madankui Tao, 7, March, 2024
    - added one input colortick_size
    Madankui Tao, 14, May, 2025
...
class Plot_2D(object):
    ...
    INPUTS:
           ...
           ## Added
           colortick_size: font size of the ticks of colorbar
           onedecimal: if==True would require have one decimal points for the colorbar label
           twodecimal: if==True would require have two decimal points for the colorbar label
    '''
    ...
    def __init__(self, var, lons=None, lats=None, lon_range=[-180,180], lat_range=[-90,90],
                 ...
                 unit="", unit_size=20, unit_bold=False, unit_italic=True, unit_offset=[0.0,0.0],
                 verbose=False,
                colortick_size=20,
                 twodecimal=None,onedecimal=None,
                ):
        ...
        # added
        self.colortick_size = colortick_size
        self.twodecimal = twodecimal
        self.onedecimal = onedecimal
    ...
    def plot(self):
        ...
        if self.colorbar:
            ...
            self.cb.ax.set_xticklabels( self.colorlabels, size=self.colortick_size )     
            
            if self.onedecimal==True:
                self.cb.ax.set_xticklabels([f"{tick:.1f}" for tick in self.cb.get_ticks()])
            else:
                pass
            
            if self.twodecimal==True:
                self.cb.ax.set_xticklabels([f"{tick:.2f}" for tick in self.cb.get_ticks()])
            else:
                pass

            # === Add a unit if specified ===
            if self.unit != "":
                self.cb.ax.text( 1.06+self.unit_offset[0], 1.0+self.unit_offset[1], self.unit,
