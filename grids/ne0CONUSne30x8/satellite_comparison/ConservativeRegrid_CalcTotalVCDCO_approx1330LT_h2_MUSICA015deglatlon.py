...
varname_filename = f'{casename}.cam.h2.MassConserve_latlon015.CO.20180701T20180801.nc'
...
for fileregion in fileregion_ls:
    ...
    SavePath = MUSICATotalVCDSave_diri+casename+'.TotalVCD1330LT_withTROPOMIAK.'+varname+'.'+fileregion+'.'+startdate+'T'+enddate+'.nc'
    if os.path.exists(SavePath):
        ...
        TotalVCD_ar = np.zeros([len(TROPOMI_days),len(regionilats),len(regionilons)])
        TotalVCD_ar[:,:,:] = np.nan
        ...
        for dateidx in range(len(TROPOMI_days)):
            ...
            regionivar_1330LT_ds = regionivar_ds.sel(time=sel_times).mean(dim='time')['CO']
            ...
            for latidx in range(len(regionilats)):
                ...
                for lonidx in range(len(regionilons)):
                    ...
                    # print(latidx,lonidx)
                    ### Get everything in total column
                    SurfP_Pa = float(regioniSurfP_1330LT_ds.PS.sel(lat=lati,lon=loni).values) # find surface pressure in Pa
                    PMID_latiloni_Pa = regioniPMID_1330LT_ds.PMID.sel(lat=lati,lon=loni).values # vertical pressure in Pa
                    T_latiloni_K = regioniT_1330LT_ds.T.sel(lat=lati,lon=loni).values # temperature in T
                    TotalAKs = vertically_gridded_AK_da.sel(lat=lati,lon=loni).values # averaging kernels
                    MUSICAvar_latiloni = regionivar_1330LT_ds.sel(lat=lati,lon=loni).values

                    totallayernum = len(MUSICAvar_latiloni) 
                    ...
                    for LAYidx, reversedLAYidx in enumerate(range(totallayernum - 1, -1, -1)):
                        # backwards from surface to TOA (currently everything is order from TOA to surface)
                        MUSICA_levi_PMID = PMID_latiloni_Pa[reversedLAYidx] # Pa
                        MUSICA_levi_TEMP = T_latiloni_K[reversedLAYidx] # K
                        AK_levi = TotalAKs[reversedLAYidx] # unitless
                        var_moleFraci = MUSICAvar_latiloni[reversedLAYidx] # mole var/mole air

                        if reversedLAYidx==(totallayernum-1):
                    ...
                    TotalVCD_ar[dateidx,latidx,lonidx] = np.nansum(np.array(latloni_AKadjusted_var_molec_cm2))
        ...
        MUSICA_TotalVCD_ds = xr.Dataset(
                            {
                                varname+'_TotalVCD': (['time','lat', 'lon'], TotalVCD_ar),
                            },
